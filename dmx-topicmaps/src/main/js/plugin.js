    ...
    components: [
      {
        ...
        props: {
          ...
          contextCommands: state => ({
            topic: [
              {label: '隐藏', multi: true, handler: idLists => store.dispatch('hideMulti', idLists)},
              {
                label: '编辑', handler: id => store.dispatch('callTopicDetailRoute', {id, detail: 'edit'}),
                disabled: isTopicEditDisabled
              },
              {label: '相关', handler: id => store.dispatch('callTopicDetailRoute', {id, detail: 'related'})},
              {label: '详情', handler: id => store.dispatch('callTopicDetailRoute', {id, detail: 'info'})}
            ],
            topic_danger: [{
              label: '删除', multi: true, handler: idLists => store.dispatch('deleteMulti', idLists),
              disabled: isTopicDeleteDisabled
            }],
            assoc: [
              {label: '隐藏', multi: true, handler: idLists => store.dispatch('hideMulti', idLists)},
              {
                label: '编辑', handler: id => store.dispatch('callAssocDetailRoute', {id, detail: 'edit'}),
                disabled: isAssocEditDisabled
              },
              {label: '相关', handler: id => store.dispatch('callAssocDetailRoute', {id, detail: 'related'})},
              {label: '详情', handler: id => store.dispatch('callAssocDetailRoute', {id, detail: 'info'})}
            ],
            assoc_danger: [{
              label: '删除', multi: true, handler: idLists => store.dispatch('deleteMulti', idLists),
