...
const actions = {
  ...
  createTopicmap ({rootState, dispatch}, {name = '未定义',
                                          topicmapTypeUri = 'dmx.topicmaps.topicmap',
                                          viewProps = {}}) {
    console.log('正在创建主题图', name, topicmapTypeUri, viewProps)
    dm5.restClient.createTopicmap(name, topicmapTypeUri, viewProps).then(topic => {
      console.log('主题图主题', topic)
      // update state
  ...
  unsetSelection ({getters, dispatch}, id) {
    ...
    if (typeof id !== 'number') {
      throw Error(`id号只能是数字, got ${typeof id} (${id})`)
    }
  ...
  createAssoc ({dispatch}, {playerId1, playerId2}) {
    ...
    console.log('建立关联', assocModel)
    dm5.restClient.createAssoc(assocModel).then(assoc => {
      console.log('已建立', assoc)
  ...
  fetchTopicmapTopics ({rootState, dispatch}) {
    ...
    if (state.topicmapTopics[workspaceId]) {
      ...
      p = dm5.restClient.getAssignedTopics(workspaceId, 'dmx.topicmaps.topicmap', true).then(topics => {
        ...
        if (!topics.length) {
          throw Error(`工作空间 ${workspaceId} 中没有主题图`)
        }
  ...
  _newTopicmap ({dispatch}, {topicmapTopic}) {
    ...
    if (!workspace) {
      console.warn('在主题图中找不到工作空间', _topicmapTopic)
      return
    }
    const topics = state.topicmapTopics[workspace.id]
    if (topics) {
      console.log('添加主题图主题', _topicmapTopic, 'to workspace', workspace.id)
      topics.push(_topicmapTopic)
      initSelection(_topicmapTopic.id, dispatch)
    } else {
      console.log('忽略主题图主题', _topicmapTopic, 'for workspace', workspace.id)
    }
  },
...
function initSelection (id, dispatch) {
  if (state.selections[id]) {
    throw Error(`'selections' state for topicmap ${id}已初始化`)
  }
...
function getTopicmapTopic (rootState) {
  ...
  if (!topicmapTopics) {
    throw Error(`尚未加载工作空间 ${workspaceId} 的主题图主题`)
  }
  const topicmapTopic = topicmapTopics.find(topic => topic.id === topicmapId)
  if (!topicmapTopic) {
    throw Error(`主题图主题 ${topicmapId} 没有找到 (workspace ${workspaceId})`)
  }
...
function firstTopicmapTopic (rootState) {
  ...
  if (!topic) {
    throw Error(`工作空间 ${__workspaceId(rootState)} 中没有主题图`)
  }
  return topic
}
