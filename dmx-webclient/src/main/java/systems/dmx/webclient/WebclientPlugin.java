...
public class WebclientPlugin extends PluginActivator implements AllPluginsActive,
    ...
    public void allPluginsActive() {
        ...
        //
        try {
            logger.info("河图洛书平台开始于 " + (System.currentTimeMillis() - time) / 1000f + " sec");
            logger.info("### 启动河图洛书Webclient: " + webclientUrl);
            Desktop.getDesktop().browse(new URI(webclientUrl));
            hasWebclientLaunched = true;
        } catch (Exception e) {
            logger.warning("### 启动河图洛书Webclient失败: " + e);
            logger.warning("### 手工启动: " + webclientUrl);
        }
    }
